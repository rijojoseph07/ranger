...
public class RangerSystemAccessControl
  ...
  public void checkCanSelectFromColumns(SystemSecurityContext context, CatalogSchemaTableName table, Set<String> columns)
  {
    ...
    for (RangerTrinoResource res : createResource(table, columns)) {
      if (!hasPermission(res, context, TrinoAccessType.SELECT)) {
        deniedColumns.add(String.valueOf(res.getValue(RangerTrinoResource.KEY_COLUMN)));
      }
    }
    if (deniedColumns.size() > 0) {
      ...
      AccessDeniedException.denySelectColumns(table.getSchemaTableName().getTableName(), deniedColumns);
    }
  }
